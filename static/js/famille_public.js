...
{% block title %}Créer une famille{% endblock %}

{% block content %}
<h2>Créer une famille</h2>

<!-- Chronomètre -->
<p>⏱ Temps de remplissage : <span id="timer">0</span> secondes</p>

<!-- Formulaire pour créer la famille -->
<form id="createFamilyForm" enctype="multipart/form-data">
    <label>Nom de la famille :</label>
    <input type="text" id="familyName" name="name" required><br>

    <!-- Photo du logement -->
    <label>📷 Photo du logement / code recensement :</label>
    <input type="file" id="housePhoto" name="photo" accept="image/*" capture="environment"><br>

    <!-- Personne racine -->
    <h3>Personne racine</h3>
    <label>Prénom :</label><input type="text" id="firstName" name="first_name" required>
    ...
    <label>Nationalité :</label><input type="text" id="nationality" name="nationality" required>
    <label>Type de pièce :</label><input type="text" id="idType" name="id_type" required>
    <label>Numéro de pièce :</label><input type="text" id="idNumber" name="id_number" required>
...
<div id="membersSection" style="display:none;">
    ...
    <form id="addMemberForm">
        <label>Prénom :</label>
        <input type="text" id="m_firstName" name="first_name" required>

        <label>Nom :</label>
        <input type="text" id="m_lastName" name="last_name" required>

        <label>Rôle :</label>
        <input type="text" id="m_role" name="role">

        <label>Date de naissance :</label>
        <input type="date" id="m_dob" name="date_of_birth">
        ...
        <input type="text" id="m_nationality" name="nationality">

        <label>Type de pièce :</label>
        <input type="text" id="m_idType" name="id_type">

        <label>Numéro de pièce :</label>
        <input type="text" id="m_idNumber" name="id_number">

        <label>Lieu de naissance :</label>
        <input type="text" id="m_placeOfBirth" name="place_of_birth">

        <label>Province :</label>
        <input type="text" id="m_province" name="province">

        <label>Ville :</label>
        <input type="text" id="m_city" name="city" value="Libreville">

        <label>Quartier :</label>
        <input type="text" id="m_district" name="district">

        <button type="submit">Ajouter membre</button>
    </form>

    <!-- Bouton géolocalisation -->
    <button id="geoBtn">📍 Géolocaliser le logement</button>
    <p id="geoResult"></p>

    <div class="mt-3">
        <a href="/page-familles" class="btn btn-outline-primary" id="finishBtn">
...
document.getElementById("createFamilyForm").addEventListener("submit", async (e) => {
    ...
    try {
        const familyResponse = await fetch("/familles/public", {
            method: "POST",
            body: formData
        });

        if (!familyResponse.ok) throw new Error("Erreur lors de la création de la famille");
...
// Ajout membre (version enrichie)
document.getElementById("addMemberForm").addEventListener("submit", async (e) => {
    e.preventDefault();
    if (!currentFamilyId) {
        alert("Créez d'abord une famille !");
        return;
    }
